...
'2016-12-27调整，避免出现双精度数出现位数丢失情况
'将手工输入的D.MMSS格式度分秒转换为十进制度便于计算
Function DMMSS2Deg(Dms As Double) As Double
Dim tmpD As Integer, tmpM As Integer, tmpS As Double
Dim tmpDms As Double
tmpDms = 0
tmpDms = Dms + 0.0000000001
tmpD = Fix(tmpDms)
tmpM = Fix((tmpDms - tmpD) * 100)
tmpS = ((tmpDms - tmpD) * 100 - tmpM) * 100
DMMSS2Deg = tmpD + tmpM / 60# + tmpS / 3600#
End Function
